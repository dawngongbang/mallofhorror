...
export default function SetupOverlay({
  ...
  if (!isMyTurn) {
    ...
    return (
      <Wrapper onShowMap={onShowMap} mapLabel="🗺️ 맵 보기" title="🎲 초기 캐릭터 배치">
  ...
  if (!game.setupDiceRoll) {
    return (
      <Wrapper onShowMap={onShowMap} mapLabel="🗺️ 맵 보기" title="🎲 초기 캐릭터 배치 — 내 차례">
        <div className="text-center">
  ...
  return (
    <Wrapper onShowMap={onShowMap} mapLabel="📍 배치하기" title="🎲 초기 캐릭터 배치 — 내 차례">
      <div className="text-center">
        {/* 주사위 표시 */}
        ...
        {showResult && (
          <>
            <p className="text-yellow-400 text-base font-bold mb-4">
              → {setupZoneOptions.map(z => ZONE_CONFIGS[z].displayName).join(' 또는 ')}
            </p>
...
function Wrapper({ children, onShowMap, title, mapLabel }: {
